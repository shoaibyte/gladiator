...
func (uc *UserCreate) AddNotebookIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddNotebookIDs(ids...)
	return uc
}

// AddNotebooks adds the "notebooks" edges to the Notebook entity.
func (uc *UserCreate) AddNotebooks(n ...*Notebook) *UserCreate {
	ids := make([]uuid.UUID, len(n))
	for i := range n {
		ids[i] = n[i].ID
	}
	return uc.AddNotebookIDs(ids...)
}

// AddSharedNotebookIDs adds the "shared_notebooks" edge to the NotebookShare entity by IDs.
func (uc *UserCreate) AddSharedNotebookIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddSharedNotebookIDs(ids...)
	return uc
}

// AddSharedNotebooks adds the "shared_notebooks" edges to the NotebookShare entity.
func (uc *UserCreate) AddSharedNotebooks(n ...*NotebookShare) *UserCreate {
	ids := make([]uuid.UUID, len(n))
	for i := range n {
		ids[i] = n[i].ID
	}
	return uc.AddSharedNotebookIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.NotebooksIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.SharedNotebooksIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
